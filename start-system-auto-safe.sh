...
echo "🚀 自動化統合システム 起動中（安全モード）..."
echo "  - tmuxマルチエージェントシステム"
echo "  - エージェント間通信機能自動有効化"
echo "  - 安全なセッション管理機能"
echo ""

# 安全なセッション管理スクリプトを読み込み
source ./tmux-safe-session-manager.sh

# 既存のセッションを安全にクリーンアップ
echo "🔄 既存セッションの安全なクリーンアップ中..."
safe_kill_session "ceo"
safe_kill_session "team"
...
echo ""
echo "🛑 システム停止（安全モード）:"
echo "  ./tmux-safe-session-manager.sh cleanup"
echo ""
echo "🎯 次のステップ:"
