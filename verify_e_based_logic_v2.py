...
def verify_e_based_logic_v2(events: Dict[str, List[float]], mouse_id: str) -> Dict:
    """E基準の判定ロジックを検証（改良版）"""
    ...
    for i, e_time in enumerate(e_events):
        ...
        # このEイベントに対応するB/Hイベントを探す
        # 次のEイベントまでの間にあるB/Hイベントが対応
        next_e_time = e_events[i + 1] if i + 1 < len(e_events) else float('inf')
        
        med_judgment = None
        med_event_time = None
        
        while bh_index < len(bh_events):
            bh_time, bh_type = bh_events[bh_index]
            
            # このB/HイベントがこのEイベントの後、次のEイベントの前にある
            if e_time < bh_time < next_e_time:
                med_judgment = bh_type
                med_event_time = bh_time
                bh_index += 1
                break
            elif bh_time >= next_e_time:
                # 次のEイベントの領域に入った
                break
            else:
                # まだ前のEイベントの領域
        ...
        trial['med_event_time'] = med_event_time
    ...
    # 結果表示
    print("\n【試行詳細】")
    print("試行 | E時刻   | 0-1秒Lick | 判定    | MED記録 | 一致 | B/H時刻")
    print("-" * 75)
    ...
    for trial in trials:
        ...
        bh_time_str = f"{trial['med_event_time']:.2f}" if trial['med_event_time'] else "---"
        print(f"{trial['trial_num']:4d} | {trial['e_time']:7.2f} | {lick_info:9s} | "
              f"{trial['my_judgment']:7s} | {trial['med_judgment']:8s} | {match_str} | {bh_time_str}")
    ...
    if mismatches:
        ...
        for trial in mismatches:
            print(f"試行{trial['trial_num']}: E={trial['e_time']:.2f}秒, "
                  ...
                  f"Lick={trial['lick_latency']:.3f}秒" if trial['lick_latency'] else "Lickなし")
...
def main():
    ...
    result1878 = verify_e_based_logic_v2(events1878, 'DT1878')
    ...
    result1899 = verify_e_based_logic_v2(events1899, 'DT1899')
    ...
    with open('/mnt/d/multiagent-system/e_based_logic_verification_v2.csv', 'w', newline='') as f:
        if all_trials:
            writer = csv.DictWriter(f, fieldnames=all_trials[0].keys())
            writer.writeheader()
            writer.writerows(all_trials)
    
    print("\n詳細データをe_based_logic_verification_v2.csvに保存しました。")
    
    # 結論
    if all(r['match_rate'] == 100.0 for r in results):
        print("\n✅ 結論: E基準の判定ロジックは完全に正しい！")
        print("   0秒=Eイベント(Go cue)、0-1秒=判定窓、判定結果はB/Hイベントとして記録")
